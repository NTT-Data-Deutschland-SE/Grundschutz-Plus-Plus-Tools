...
**NEU** [Schulungsvideo zu OSCAL, GS++ und den Tools](https://youtu.be/nqLAiSsXqqA)
...
* **Vollständige Zielobjekt-Profile, deterministisch ergänzt:** Hat der Baustein eine Zielobjektkategorie, enthält das Profil **alle Controls ihres Pools** — nicht nur die mit KI-Treffer (Prozess-Bausteine und C.2 bleiben beim Mapping-Ergebnis). Der Ed2023-Wortlaut hängt als `imports`/`modify`/`alters`-Ergänzung an den Controls: aus dem KI-Satz-Mapping und zusätzlich aus dem **vorberechneten G++↔ED23-Matching** (`hilfsdateien/gpp_ed23_anforderungen.json`, gepinnte Quelle) — dessen Treffer werden mit dem vollen Anforderungstext aus dem geparsten PDF eingesetzt und tragen das Präfix „vorberechnetes G++-Matching"; der Text kommt immer aus dem PDF, nie aus der Hilfsdatei.
* **Profil-Editor (Stufe 3):** Das erzeugte Profil ist nicht nur JSON, sondern eine bearbeitbare Liste — Titel ändern, Controls abwählen oder aus dem Katalog aufnehmen, die je Control mitgeführte Ergänzung (der verbatim gemappte Ed2023-Text als `alters`) redigieren. **Vorhandene Profile aus dem Arbeitsstand** lassen sich über die Auswahl im Input-Bereich ohne Pipeline-Lauf öffnen und weiterbearbeiten — gespeichert wird in denselben Set-Eintrag; bloßes Öffnen ändert nichts. Abgewählte Controls bleiben gedimmt sichtbar und lassen sich per Häkchen zurückholen. Die JSON-Sicht bleibt daneben bestehen; Änderungen dort gelten nach „JSON übernehmen". Es gibt genau eine Wahrheit (`state.profileDef`): Download, Set-Ablage und beide Sichten lesen dasselbe Objekt, und jede Änderung fließt entprellt in den Arbeitsstand zurück — der Chip neben der Stufe zeigt, ob das gelungen ist. Scheitert die Ablage (in der Zusammenarbeit typisch: das Set ist nicht gesperrt), erscheint das rote Banner des Kerns, nicht nur eine Logzeile.
...
**[gpp-core.js](./gpp-core.js) — die geteilte Bibliothek.** Konfiguration, Artefaktspeicher, Prompt-Registrierung, Quellen-Pins und ein ZIP-Writer, einmal vorhanden statt neunmal kopiert. Die Datei wird als Geschwisterdatei eingebunden und funktioniert ohne Server, auch per `file://`. **Damit ist die Sammlung nur noch als Ganzes lauffähig:** Eine einzelne HTML-Datei weiterzugeben genügt nicht mehr — es braucht den Ordner beziehungsweise `GS++-oscal-app.zip`. Fehlt `gpp-core.js`, sagt das jede Anwendung beim Start deutlich, statt still zu scheitern. Daneben liegt **[gpp-core.css](./gpp-core.css)** mit den gemeinsamen Design-Tokens (Farben) und den Scrollbalken — vorher trug jede Seite dieselben Werte als Kopie; ein helles Farbschema hat damit genau einen Schalter. Fehlt es, sind alle Seiten grau und der Kern meldet das in der Konsole. Aus dem Kern kommt auch der **Listenfilter**: Listen mit mehr als zehn Einträgen (Zielobjekte und Control-Quellen im Generator, Komponenten im Editor, Artefakte in der Übersicht) beginnen mit einem Suchfeld, das die Einträge nach Text ein- und ausblendet; der Zähler zeigt Treffer / gesamt, Escape leert das Feld. Seit dem Nachtrag zu Issue #40 trägt es auch das **helle Farbschema**: Umschalten über den Knopf „◐“ in der Kopfzeile der Übersicht (Dunkel → Hell → System) oder in den Einstellungen unter „Darstellung“; die Wahl gilt für alle Werkzeuge dieses Browsers, „System“ folgt dem Betriebssystem. Seit Kern v4 (Release 4.1) liegt hier auch die **KI-Schicht** (`gppAi`): eine Backend-Weiche für Gemini, OpenRouter und den eigenen Endpoint mit JSON-Schema-Antworten, Retry, Abbruch, Worker-Pool, Chunking und Ergebnis-Cache in IndexedDB — dazu die **Textextraktion** aus PDF, DOCX, Text, JSON, XML und HTML (`gppDocText`; pdf.js und mammoth kommen erst bei Bedarf vom CDN). SSP-Generator und SSP-Editor nutzen sie statt eigener Kopien, die übrigen Werkzeuge folgen.
...
* **Asset-Management**: Sie integrieren Muster-Assets oder laden eigene Zielobjekte direkt aus der GitHub-Bibliothek — oder legen ein Zielobjekt aus einem **Profil im aktiven Arbeitsstand** an („4. Zielobjekte anlegen" → „Aus dem Arbeitsstand"). Die Controls des Profils landen beim Zielobjekt und stehen im Tailoring wie jede andere Quelle; mitgebrachte Ergänzungen (`alters`, etwa der Ed2023-Quelltext aus „BSI → G++ Profil") werden als Anpassungen übernommen und beim Export unverändert wieder ausgegeben. Baustein und Zielobjektkategorie des Profils bestimmen den OSCAL-Komponententyp. Die vierte Quelle in „4. Zielobjekte anlegen" ist eine **BSI-Komponente** (Component Definition aus dem `implementation_layer` der Stand-der-Technik-Bibliothek, z. B. Keycloak oder Netzarchitektur): jedes Bauteil der CDef wird ein eigenes Zielobjekt mit Name, Status und Schutzbedarf wie jedes andere — dieselbe CDef darf mehrfach angelegt werden (zwei Keycloak-Instanzen, drei Netzsegmente), der eigene Name unterscheidet die Instanzen, ohne Namen wird hochgezählt; seine Controls sind die auf G++ gemappten Anforderungen der CDef. Der Vorlagentext der CDef erscheint im SSP-Editor nur als Referenz über dem Kommentar, der Umsetzungsstatus startet mit „Offen" — die eigene Umsetzung dokumentiert der Bearbeiter selbst.
* **Einfügen eigener Profiles**: Die mit [BSI 2 Profile](./Baustein_2_Profile.html) erstellten Profile hinzufügen — entweder als Zielobjekt (siehe oben) oder unter „2. Zusätzliche Kataloge" als Import in den SSP; beides aus dem Arbeitsstand, ohne Umweg über Download und Datei. Das eigene Blaupausen-Profil des Generators wird dabei nicht angeboten (es wäre zirkulär).
...
* **Ein Abschnitt zur Zeit**: Die Navigation in der linken Leiste zeigt immer genau einen Abschnitt — Metadaten, Charakteristika, Risikoanalyse, Excel-Austausch, Ressourcen-Status oder den Grundschutzcheck mit dem gewählten Zielobjekt. Suche und Filter führen in den Grundschutzcheck; die zuletzt gewählte Ansicht bleibt beim Neuladen erhalten.
...
* **Excel-Austausch**: Über den Abschnitt „Excel-Austausch" (Sprungmarke oben in der Navigation der Seitenleiste) geht der gesamte Grundschutzcheck als `.xlsx` heraus — eine Zeile je Control und Komponente, mit Auswahlliste für den Status, aufgelösten Parametern im Anforderungstext, einem Blatt für die Tailoring-Parameter und einem Blatt mit der Ausfüllhilfe. Wer die Antworten hat, muss das Werkzeug nicht öffnen: ausfüllen, zurückschicken, dieselbe Datei wieder einlesen. Zugeordnet wird über Control-ID und Komponenten-UUID, Spalten dürfen also umsortiert und Zeilen gefiltert werden. Übernommen wird nur, was gültig ist — eine Zeile mit unzulässigem Status, unbekanntem Reifegrad oder fremder Control-ID wird komplett übersprungen und in der Log-Konsole namentlich genannt, nie halb eingetragen. Fehlt eine Spalte in der zurückgegebenen Datei, bleibt das zugehörige Feld im SSP unverändert.
* **Nachweis prüfen (KI, Issue #41)**: Am Kopf jeder Asset-Karte lädt „⎘ Nachweis prüfen“ ein oder mehrere Nachweisdokumente (PDF, DOCX, Text, Markdown, CSV, JSON, XML, HTML) und gleicht sie mit allen Controls des Zielobjekts ab. Das Modell nennt je Control Status, Konfidenz, einen Umsetzungstext aus dem Belegten und wörtliche Zitate mit Fundstelle — belegen mehrere Stellen dasselbe, werden alle genannt. Controls ohne Zitat oder unter der Mindest-Konfidenz sind kein Vorschlag; ein Nachweis deckt nie alles ab. Vor dem Schreiben zeigt eine Review-Tabelle die Vorschläge: Status, Reifegrad und Kommentar je Zeile änderbar, Zeilen abwählbar; bestehende Kommentare bleiben stehen und bekommen den KI-Text angehängt (umschaltbar). Übernommen wird mit derselben Semantik wie Formular und Excel-Import; je Datei entsteht eine Resource in der `back-matter` (Titel, SHA-256, Größe, Medientyp, `rlink` auf den Dateinamen — die Datei selbst bleibt beim Anwender) und am `by-component` ein Link `rel="evidence"`, den die Karte als Chip zeigt. Die Dokumente bleiben in der Sitzung geladen und lassen sich nacheinander gegen mehrere Zielobjekte prüfen; Ergebnisse werden je Dokumentteil gecacht. Dokumentinhalte gelten dem Modell ausdrücklich als Daten, nie als Anweisung; die Prüfung vor dem Schreiben ist die zweite Sicherung. Backend, Modell und Key kommen aus `config.html`, die beiden Prompts sind dort editierbar.
* **AI-Assistent**: Die Anwendung nutzt KI-Unterstützung für tiefere Einblicke (Backend, Modell und Key aus `config.html`):
