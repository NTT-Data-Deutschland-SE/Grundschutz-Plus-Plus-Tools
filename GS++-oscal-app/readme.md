...
* **Asset-Management**: Sie integrieren Muster-Assets oder laden eigene Zielobjekte direkt aus der GitHub-Bibliothek — oder legen ein Zielobjekt aus einem **Profil im aktiven Arbeitsstand** an („4. Zielobjekte anlegen" → „Aus dem Arbeitsstand"). Die Controls des Profils landen beim Zielobjekt und stehen im Tailoring wie jede andere Quelle; mitgebrachte Ergänzungen (`alters`, etwa der Ed2023-Quelltext aus „BSI → G++ Profil") werden als Anpassungen übernommen und beim Export unverändert wieder ausgegeben. Baustein und Zielobjektkategorie des Profils bestimmen den OSCAL-Komponententyp. Die vierte Quelle in „4. Zielobjekte anlegen" ist eine **BSI-Komponente** (Component Definition aus dem `implementation_layer` der Stand-der-Technik-Bibliothek, z. B. Keycloak oder Netzarchitektur): jedes Bauteil der CDef wird ein eigenes Zielobjekt mit Name, Status und Schutzbedarf wie jedes andere; seine Controls sind die auf G++ gemappten Anforderungen der CDef. Der Vorlagentext der CDef erscheint im SSP-Editor nur als Referenz über dem Kommentar, der Umsetzungsstatus startet mit „Offen" — die eigene Umsetzung dokumentiert der Bearbeiter selbst.
