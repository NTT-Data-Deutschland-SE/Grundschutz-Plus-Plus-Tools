...
* **Asset-Management**: Sie integrieren Muster-Assets oder laden eigene Zielobjekte direkt aus der GitHub-Bibliothek — oder legen ein Zielobjekt aus einem **Profil im aktiven Arbeitsstand** an („4. Zielobjekte anlegen" → „Aus dem Arbeitsstand"). Die Controls des Profils landen beim Zielobjekt und stehen im Tailoring wie jede andere Quelle; mitgebrachte Ergänzungen (`alters`, etwa der Ed2023-Quelltext aus „BSI → G++ Profil") werden als Anpassungen übernommen und beim Export unverändert wieder ausgegeben. Baustein und Zielobjektkategorie des Profils bestimmen den OSCAL-Komponententyp.
