...
Zwei weitere Regeln aus derselben Logik: **Neue Sets legt nur der Admin an** (Nicht-Admins schreiben und sperren nur Sets, die existieren — Artefakte vorhanden oder Set-Recht vergeben; der „+ Neues Set"-Knopf erscheint entsprechend nur für Admins, im reinen Lokalbetrieb ohne Datenbank bleibt er frei). Und **gelöscht heißt überall gelöscht**: Löscht jemand ein Set oder einzelne Artefakte, spiegelt der nächste Abgleich die Löschung auf alle anderen Browser — eigene, noch nicht übertragene Arbeit bleibt dabei geschützt, und rein lokale Sets, die nie auf dem Server waren, rührt der Abgleich nicht an.
