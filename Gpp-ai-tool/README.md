...
| 6 | `stage_ed23_anforderungen` | **Yes** | For every G++ control, finds the **precisely** matching BSI ED2023 Anforderungen (grounded on a cached, stripped ED2023 corpus with numbered sentences; returned IDs and sentence numbers are validated against the real catalog). The prompt carries the control's full context — param-resolved statement, guidance, and the sibling controls of the same Praktik as explicit negative context — and every match names the ED23 sentence it rests on (`(Satz n)` in the Begründung, `statement-sentence` prop in OSCAL). Writes the OSCAL mapping collection `hilfsdateien/gpp_ed23_anforderungen.json`, which the GS++-oscal-app use for the "Zeige BSI ED23 Anforderungen" panel. |
